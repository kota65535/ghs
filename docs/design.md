...
ghs apply [-f .github/settings.yml] [-R owner/repo] [--format text|markdown|json] [--allow-unknown]
```
...
ルールセットの `rules` の要素は 23 通りの `oneOf` で、それ自体の `properties` を持たない。

### 配列要素の定義: 要素の `type` で引く

`rules` の各要素が何を受け付けるかは、要素自身の `type` が決める。`pull_request` なら `required_approving_review_count` を、`required_status_checks` なら `required_status_checks` を取り、両者は排他である。そこで `oneOf` の分岐をマージせず、各分岐の `title`（＝その分岐を選ぶ `type` の値）をキーとして `Field.Variants` に持つ。要素が 1 形状しかない配列（environments の `reviewers` 等）は空文字列をキーとし、要素が `type` を名乗っていてもその 1 形状で扱う。

これは 2 つに効く:

- `ghs init` が各ルールの説明と各パラメータの説明をコメントとして書き出す。`required_approving_review_count: 2` が、その意味を述べた行の下に書かれる
- ロード時の検証が `rules` の中まで届く。タイポは `rulesets["protect-main"].rules[0].parameters.required_approving_review_cont` として報告される

**記述にない `type` も弾く。** enum の値と同じ扱いで、受け付ける型の一覧を添えて報告する。GitHub はリリースの合間に新しいルール型を追加するので、これは「存在しない型」と「ghs がまだ追いついていない型」の両方を弾くことになるが、素通しの例外を設けるより、ピン留めした記述の追随を速くして待ち時間そのものを潰す方を採る。

`ghs init` も、記述にないルール型に当たったらファイルを書かずに止める。init はほとんどのユーザーにとって最初の一歩なので、ここで黙って進む選択肢は 2 つともファイルを壊す。報告どおりに書けば plan が読めないファイルになり、落とせば実在するルールがファイルから消え、apply がそれを削除と読む。どちらも気付かれずに起きるより、その場で止めて ghs の更新を促す方がよい。
...
### 更新 API を持たないコレクション: autolinks

autolink は一覧・作成・個別取得・削除しか持たない。既存の autolink の `url_template` を変えるエンドポイントが無いので、`Collection.Update` は **delete してから create** で実装する。delete が先なのは `key_prefix` が一意だからで、消す前に作れば同じ prefix が 2 つになって API に拒否される。途中で落ちたときに残るのは「autolink が無い状態」になり、次の apply が作り直す。

この形には差分表示が嘘になる危険が 1 つある。create は autolink 全体を述べるので、宣言だけで作り直すと、`url_template` だけを書き換えたときに宣言していない `is_alphanumeric` が既定値に戻る。plan が報告していない変更が起きるということで、「宣言したキーだけを管理する」という部分管理のセマンティクスにも反する。そこで作り直しのボディは、**現在値のうち宣言可能なフィールドに宣言を上書きしたもの**にする。plan と apply の一致はこれで保たれる。

突き合わせのキーは `name` ではなく `key_prefix` になる。autolink には名前という概念が無く、`key_prefix` を name と呼ぶのは API に無いフィールドを発明することになる。そこで `schema.Node.Key` に「この コレクションの要素を識別するフィールド」を持たせ、空なら従来どおり `name` とする。`gen/main.go` の operations でそれを述べ、config の突き合わせ・resource の `byName`・plan の表示がそこから引く。宛先は rulesets と同じく API が発行した `id` で、両者が同じ形なので `pathByID` に括り出した。

プラン制限も他と違う。autolinks は GitHub Free に無く、その場合の一覧は 403 を返す。既定の `doesNotApply`（409 / 422）には **403 を足さない**。「読めない」と「そこに無い」は違うという方針は 403 について明記してあり、権限の足りないトークンが設定を「無い」と報告するようになってしまう。403 が不在を意味すると言えるのはこのエンドポイントだけなので、その判断は `resource.Autolinks` に置く。init も同様に、条件付きコレクションが何も報告しなければキーごと書かない。`autolinks: []` は「autolink を 1 つも持たない」という宣言であって、持てないリポジトリについての記述ではない。
