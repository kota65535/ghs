...
//
// Source: github/rest-api-description@c6721f32a17a71397ae46be21be90d7f1a173b6e
...
var generated = Node{
	...
	Fields: map[string]Field{
		"allow_auto_merge":             {Type: "boolean", Description: "Either `true` to allow auto-merge on pull requests, or `false` to disallow auto-merge.", Default: false},
		"allow_forking":                {Type: "boolean", Description: "Either `true` to allow private forks, or `false` to prevent private forks.", Default: false},
		"allow_merge_commit":           {Type: "boolean", Description: "Either `true` to allow merging pull requests with a merge commit, or `false` to prevent merging pull requests with merge commits.", Default: true},
		"allow_rebase_merge":           {Type: "boolean", Description: "Either `true` to allow rebase-merging pull requests, or `false` to prevent rebase-merging.", Default: true},
		"allow_squash_merge":           {Type: "boolean", Description: "Either `true` to allow squash-merging pull requests, or `false` to prevent squash-merging.", Default: true},
		"allow_update_branch":          {Type: "boolean", Description: "Either `true` to always allow a pull request head branch that is behind its base branch to be updated even if it is not required to be up to date before merging, or false otherwise.", Default: false},
		"archived":                     {Type: "boolean", Description: "Whether to archive this repository. `false` will unarchive a previously archived repository.", Default: false},
		"default_branch":               {Type: "string", Description: "Updates the default branch for this repository."},
		"delete_branch_on_merge":       {Type: "boolean", Description: "Either `true` to allow automatically deleting head branches when pull requests are merged, or `false` to prevent automatic deletion.", Default: false},
		"description":                  {Type: "string", Description: "A short description of the repository."},
		"has_issues":                   {Type: "boolean", Description: "Either `true` to enable issues for this repository or `false` to disable them.", Default: true},
		"has_projects":                 {Type: "boolean", Description: "Either `true` to enable projects for this repository or `false` to disable them. **Note:** If you're creating a repository in an organization that has disabled repository projects, the default is `false`, and if you pass `true`, the API returns an error.", Default: true},
		"has_pull_requests":            {Type: "boolean", Description: "Either `true` to allow pull requests for this repository or `false` to prevent pull requests.", Default: true},
		"has_wiki":                     {Type: "boolean", Description: "Either `true` to enable the wiki for this repository or `false` to disable it.", Default: true},
		"homepage":                     {Type: "string", Description: "A URL with more information about the repository."},
		"is_template":                  {Type: "boolean", Description: "Either `true` to make this repo available as a template repository or `false` to prevent it.", Default: false},
		...
		"use_squash_pr_title_as_default": {Type: "boolean", Description: "Either `true` to allow squash-merge commits to use pull request title, or `false` to use commit message. **This property is closing down. Please use `squash_merge_commit_title` instead.", Default: false},
		"visibility":                     {Type: "string", Enum: []string{"private", "public"}, Description: "The visibility of the repository."},
		"web_commit_signoff_required":    {Type: "boolean", Description: "Either `true` to require contributors to sign off on web-based commits, or `false` to not require contributors to sign off on web-based commits.", Default: false},
	},
			...
			Fields: map[string]Field{
				"bypass_actors": {Type: "array", Description: "The actors that can bypass the rules in this ruleset", Variants: map[string]Field{
					"": {Type: "object", Description: "An actor that can bypass rules in a ruleset", Fields: map[string]Field{
						...
						"bypass_mode": {Type: "string", Enum: []string{"always", "exempt", "pull_request"}, Description: "When the specified actor can bypass the ruleset. `pull_request` means that an actor can only bypass rules on pull requests. `pull_request` is not applicable for the `DeployKey` actor type. Also, `pull_request` is only applicable to branch rulesets. When `bypass_mode` is `exempt`, rules will not be run for that actor and a bypass audit entry will not be created.", Default: "always"},
					}},
				}},
				...
				"rules": {Type: "array", Description: "An array of rules within the ruleset.", Variants: map[string]Field{
					...
					"code_coverage": {Type: "object", Description: "Enforce minimum line coverage thresholds on pull requests. When configured, uploaded coverage data must meet the specified criteria before changes can be merged.", Fields: map[string]Field{
						"parameters": {Type: "object", Fields: map[string]Field{
							"max_coverage_drop": {Type: "number", Description: "The maximum percentage points that line coverage may drop relative to the default branch. Pull requests that reduce line coverage by more than this amount will be blocked."},
							"minimum_coverage":  {Type: "number", Description: "The absolute minimum line coverage percentage required. Pull requests with line coverage below this threshold will be blocked."},
						}},
						"type": {Type: "string", Enum: []string{"code_coverage"}},
					}},
					"code_quality": {Type: "object", Description: "Choose which severity levels of code quality results should block pull request merges. When configured, a code quality analysis must be done on the pull request before the changes can be merged.", Fields: map[string]Field{
						"parameters": {Type: "object", Fields: map[string]Field{
							"severity": {Type: "string", Enum: []string{"all", "errors", "notes", "warnings"}, Description: "The lowest severity level at which code quality reviews need to be resolved before commits can be merged."},
						}},
						"type": {Type: "string", Enum: []string{"code_quality"}},
					}},
